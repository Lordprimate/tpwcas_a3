...
tpwcas_running = true;
	...
	if(isNil "tpwcas_minskill") then {
		tpwcas_minskill = 0.75; // minimum 30% of original skill
		};  
	...
	//CHECK IF ASR_AI 0.9.31.92 OR GREATER IS RUNNING       
	if (isClass (configFile >> "cfgPatches">>"asr_ai3_main")) then    
	{   
		_asr_ai_va = getArray (configFile>>"cfgPatches">>"asr_ai3_main">>"versionAr");  
		if (_asr_ai_va select 0 >= 0 && {_asr_ai_va select 1 >= 9} && {_asr_ai_va select 2 >= 31} && {_asr_ai_va select 3 >= 92}) then   
		{  
			//DISABLE REVEAL
			tpwcas_reveal = 0;
...
//	};
	...
	if ( (tpwcas_los_enable isEqualTo 1) && (isServer) ) then 
	{
		...
		if !(isMultiPlayer) then 
		{
			[tpwcas_los_fnc_mainloop, 0] call cba_fnc_addPerFrameHandler; // SP Usage
		}
		else 
		{
